#### 작업예정.
- 스프링 프로젝트 순서
- 1.JUnit > 마이바티스(DB핸들링) > AOP(다중게시판기능) > 페이징처리 > 검색기능 > 트랜젝션(게시판) > 첨부파일기능(파일업로드/다운로드) >  스프링시큐리티(로그인기능/권한체크)
- 2. > 댓글처리(RestApi-백엔드, Ajax처리-프런트단) > 네이버아이디로그인(외부Api사용) > heroku 클라우드 배포.
- 3. 문서작업(화면기획서Excel 제작, 화면설계서PPT)
...
#### 20210609(수) 작업.
- 프로젝트를 진행: 보유기술70% + 신기술30%(개발사도 모름)-9할 실패.
- PageVO.java 클래스 생성 마무리OK
- JUnit에서 위작업한 내용을 기준으로 selectMember() 테스트진행 OK.(검색,페이징)
- <![CDATA[ 쿼리 ]]> : 태그 안쪽의 부등호를 사용하기 위해서 문자열 변환 태그를 사용.
- 쿼리에서 변수와 문자열과의 연갈할때는 +(자바)X, ,(x),||(O)
- JUnit에서 회원관리 나머지 CRUD 테스트진행
...
#### 20210602(수) 작업.
- 예외처리하는 목적: 에러상황에서도 프로그램이 중단되지 않도록 하는 것이 주 목적.
- 스프링에서는 개발자가 처리하는경우는 100개중 1~2개 정도이고, 대부분 throws Exception 스프링으로 넘김. 
- 쓰레드: thread, 실행되는 프로그램을 명시, 1개 프로그램에는 보통1개쓰레드가 실행(수업소스는 여기까지), 특이한 경우(http네트워크프로그램인경우-게시물 다운로드버튼 클릭시 다운로드 쓰레드가 실행 되면서, 리스트버튼을 클릭했을때 목록보기 쓰레드가 동시에 실행-안드로이드앱에서필요) 여러개의 쓰레드가 필요할 수 있습니다.
- 예외처리 : Throwable 오브젝트가 실행시 에러가 발생하면, 예외(내용)를 보낼때(던질때) 사용하는 클래스 Throwable 클래스 입니다.
- 스프링에서는 예외(에러)정보를 스프링처리 합니다.
- 자바앱에서는 예외(에러)정보를 개발자가 처리합니다.(개발자 손이 많이 갑니다.) 지금 실습.
- 예외처리 : 에러발생시 프로그램이 중지되는 것을 방지하고, 계속 프로그램을 사용할 수 있도록 처리하는 방법 = Exception
- 패키지는 관련기능을 한곳에 모아서 개발자가 관리하기 편하게 하기위해서 구분한 이름(폴더명)
...
- MVC웹프로그램 차이점=URL직접접근 불가능(보안위험성 낮음)-관공서,대학,은행,등 사용
- MVC프로젝트에도 직접접근이 가능한 resources폴더 직접접근 가능.-static 컨텐츠(html,css,js)를 모아 놓은 폴더. views폴더 jsp는 직접접근이 불가능.
