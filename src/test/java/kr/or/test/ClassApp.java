...
/**
 * 이 클래스는 1)클래스상속(extends)연습. 2)오브젝트(객체)생성에 필요한 new 키워드(예약어) 생성자메서드 사용 연습.
...
class Circle {
	private int r;// r= 써클(원)의 반지름으로 사용할 변수
	// 원을 만드는 메서드(아래)
	// puvlic Circle() { } 만들지 않아도 자동으로 만들어 집니다.
...
class Salesman extends Employee {// extends확장 => 상속
	public Salesman() {// 클래스명과 똑같은 이름의 메서드가 생성자 함수입니다.
		...
		mDept = "판매 부서";// 부모클래스의 변수를 상속받아 사용가능 합니다.
	}
...
// 추상클래스를 사용하는 방법(아래)
// 스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용합니다. 사용의도는 비슷합니다.
class Traiangle extends GraphicObject {
	//삼각형 그리기
	@Override // 부모클래스의 메서드를 재정의해서 사용=>*오버라이드 / *오버로드 차이점=Step1.java(중요)
	void draw() {
		// 삼각형을 만듬.
		System.out.println("  *  ");
		System.out.println(" * * ");
...
class Rectangle extends GraphicObject {
	//사각형 만들기
	@Override //오버라이드@애노테이션 이 있으면, 상속이라는 뜻.
	void draw() {
		System.out.println("*****");
		System.out.println("*****");
		System.out.println("*****");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		GraphicObject traiangleObject = new Traiangle();//추상클래스는 객체로 만들지못함.
		GraphicObject rectangleObject = new Rectangle();
		traiangleObject.draw();
		rectangleObject.draw();
		// 개발자가 입력한 반지름의 원의 넓이를 구하는 오브젝트를 생성(아래)
		Circle circle = new Circle(5);// 반지름이 5인 원의넓이를 구하는 객체생성
