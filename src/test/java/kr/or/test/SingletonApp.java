 ...
 * 장점: 메모리 낭비를 방지하는 효과
 * 예, DB접속 객체를 예로들면.
 * Connection conn1 = dataSource.getConnection();//싱글톤 객체  new키워드를 사용하지 않습니다.
 * Connection 객체가 생성돼 있으면,
 * Connection conn2 = dataSource.getConnection();//싱글톤 객체
 * Connection conn3 = dataSource.getConnection();//싱글톤 객체
 * conn1==conn2==conn3 다같은 동일객체로 같은 메모리를 사용합니다. 즉 1개의 객체만 생성됩니다.
 * 결과적으론 1번 DB커넥션이 성공하면, 객체가 없어질때까지 (null) 중복으로 생성되지 않습니다.
 * 즉, 객체를 2번 생성할 필요가 없는 로직일때 사용
 * 대표적인 싱클톤객체는 Calendar(달력)클래스, DB접속클래스, Logger(로깅)클래스가 있습니다.
 * @author 조준호
 *
 */
