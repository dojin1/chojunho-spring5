...
- 관리자단 AdminLTE적용- UI시간: (게시판CRUD-html, 대시보드html)s
...
- html5 내장된 유효성검사? = required(공백체크), type="email"(이메일형식체크),type="password"(입력값을 숨김처리),type="number"(숫자체크)
- js로 유효성검사? = 비밀번호 확인(2개입력값체크)
- 반응형 서브페이지를(로그인,회언가입,마이페이지).
- HTML5부터는 유효성검사가 내장 되어있기 때문에, JS로 빈칸인지 코딩할 필요가 없습니다 required 속성을 태그에 집어넣으면 끝.
- 기획시, 메인 CSS 네이밍, 서브1 CSS 네이밍.
- 과장/이사급이 UI를 2개파일 만듬 또는 테마포레스트 같은 디자인UIhrml+css+js를 구매 -> 기반으로 퍼블리셔, 프론트개발자가  CSS나 HTML작업을 이어서 합니다.
- 금액이 4000~2억 정도의 프로젝트는 자체 UI를 사용.

#### 20210525(화) 작업내역.
- css: 계층:조부>부>본인(timeling)>자식(collapse)>손자(time-label)>증손자
...
- 위 시작전 이클립스에서 프로젝트(윈도우탐색기와동일구조) 익스플로러와 패키지 (폴더경로를 . 으로 구분) 익스플로러 차이 확인.

#### 20210524(월) 작업내역.
- 시간 형식(Type): date(년 월 일), dateTime(년 월 일 시 분 초), TimeStampt(년 월 일 시 분 초)
- 1970년 생일: 타임스탬프가 1970년 1월1일 부터 현재까지의 초를 계산한 결과 값.
- 1970년 1월1일 부터 16억초2천..초가 흐른시간이 현재시간.
- 부트스트랩용어: grid(12칸), modal, xs,sm,md,lg, bs(부트스트랩약자),fa-폰트어썸
- 팝업창: 모달창(modal-필수창:작업후만 다음으로가능), 모달리스창(modeless-작업과 상관없이 다른창으로 이동가능),
- 폰트어썸(아이콘웹폰트)장점:아이콘을 확대해도 깨지지않음(벡터이미지방식의 아이콘)
- 포토샵 디자인으로 아이콘을 만들면 확대시 깨짐.(비트맵=스칼라방식의 아이콘 벡터와반대.)
- 부트스트랩 그리드레이아웃 : 화면을 12개의 컬럼으로 분리해서 크기를 반응형으로 만듭니다.
- row(가로줄), cols(세로칸), col-md-12(화면가로 크기를 12로지정 = 100%,조건md(미디엄에서 100%))
- 반응형의 화면크기: .col-xs엑스트라스몰(~750px), sm(750px~), md(970px~), lg(1170px)
- col-6 (화면가로 크기를 6으로지정 = 50%)
- board_view.html 댓글 UI디자인(부트스트랩) 추가.
- 반응형 서브페이지들(로그인, 회원가입, 마이페이지).
- 관리자단 AdminLTE적용.(회원관리CRUD, 게시판CRUD, 대시보드ㄴ)
- 이클립스 헬로월드 실습. 1. 다이나믹 웹프로젝트(jsp만드는 방식) 만든후 사용자단 UI실행만 하고, 삭제.
- jsp(1세대 2000년게시판) -> 서블렛(2세대(2005년)servelet게시판)-> 스트러츠(3세대(2010)프레임워크)-> 스프링(4세대(2013년)프레임워크)
...
- 글쓰기폼(부트스트랩을 적용): 첨부파일,내용입력부분 웹에디터 추가.
...
- board_write.html 파일에 bootstrap코어 + 서머노트플러그인 임포트
- board_view.html 댓글 UI디자인(부트스트랩) 추가
- 반응형 서브페이지(로그인,회원가입,마이페이지)
- 관리자단 AdminLTE적용. (회원관리CRUD, 게시판CRUD, 대시보드)
- 톰캣(jsp해석) vs VScode 라이브서버(아파치-html해석)
- 자바소스(.java앱용,jsp웹용) -> 컴파일(인터프리터X) -> class파일(여기에 DB자료가 동적으로 입출력됨.) -> html번역(WAS-톰캣) -> 크롬(IE) 화면에 렌더링 결과
- localhost 도메인  =127.0.0.1 아아피와 바인딩되는 도메인
- 이클립스 hello world 실습. (아래)
- 백엔드 :1. 이클립스(스프링-규모가있는프로젝트), 2.인텔리J
- 1.다이나믹 웹프로젝트(jsp만드는방식) 만든후 사용자단UI 실행만 하고 삭제.
- 2.스프링 (MVC)웹프로젝트 만들예정. 헬로월드1개 - 2달간 이프로젝트 진행됩니다.
- 위 스프링 프로젝트에서 자바 기초도 실습예정 -> htmlUI 만든것을 jsp변경작업함.

#### 20210520(목) 작업내역
- 메인페이지 시간이 걸리는부분(프론트엔드): 메뉴처리, 슬라이딩처리 OK
- 메인페이지 시간이 걸리는부분(백엔드): 최근갤러리, 최근공지사항 등 DB데이터를 출력하는부분
- 모바일 게시판페이지(CRUD) CSS처리 : Read(list, view페이지) OK
- Read: 리스트형식(다중map):board_list.html 예: 회원리스트(회원목록)
- Read-단일Map형식, (key:value1, key2:value2,key3:value3 ...): voard_view.html (회원상세보기)
...
- (데이터변수) 바인딩: 정적인 컨텐츠에 동적인 데이터를 넣어주는 것을 바인딩이라고함. 바인딩=묶어주다.
- 오후에 자바(스프링)개발환경설치예정.(이클립스:전자정부표준프레임워크의 개발환경툴 설치)
- 전자정부표준프레임워크를 제작한 업체 : 삼성SDS, SK C&C, LG CNS 3사 협업으로 제작.
- 게시물 타이틀(제목) 글넘침 부분 CSS처리 OK , 스프링가서는 jsp 프로그램으로 처리예정.
- 모바일 서브페이지 CSS 스타일처리
- 테블릿+PC용 CSS 스타일처리
- 테블릿+PC용 CSS 스타일처리
- 모바일+테블릿+PC 댓글시스템 CSS+제이쿼리+부트스트랩 처리
- AdminLTE(부스트랩기반템플릿-반응형)를 이용해서 관리자단 디자인 만들기
- UI디자인 끝 ------------------------------------------
...
- 반응형 페이지의 핵심기술은 미디어쿼리 명령어 사용, 가로크기를 100%(100px고정크기가 아닌 비율(%))로 내부 컨텐츠 크기를 지정하는 방법)
- 미디어(PC화면, 스마트폰화면, 프린터, 태블릿화면) + 쿼리(질의어-질문)
- @media 미디어타입(screen, print 등=all) and (min-width:801px)최소801px부터 {구현내용}
- 태블릿은 마우스오버 기능을 넣을 필요가 없다. (손가락으로 선택을 하기때문에)
- 배치1: jQuery코어 임포트 이후에 사용자가 지정한 js 배치를 해야합니다.(아래설명)
- 배치2: reset.css, mobile.css 임포트 이후에 사용자가 지정한 tablet.css, pc.css 를 배치해야 레이아웃이 깨지지 않습니다.
...
- 제이쿼리적용부문: 메뉴OK, 슬라이드이미지3개 처리: 모바일 페이지만 마무리
- 보통 이미지 슬라이드 처리는 외부 라이브러리(Lib) 사용(니보슬라이드, 캐러셀슬라이드)
- 외부Lib 사용안하고 우리가 만들어봅니다.OK
...
- top상단이동 OK.
- 과제물 제출 준비
- jQery Json 데이터 파싱 실습
- 8교시에 과제물 다음카페로 제출

#### 20210513(목) 작업내역
- 픽사베이 이미지 3개: 로고1, 슬라이드 이미지1, NoImage 1 받고, 경로 적어놓기
- 로고 이미지:https://cdn.pixabay.com/photo/2021/04/28/23/58/ice-6215207_960_720.jpg 
- 슬라이드 이미지: https://pix4free.org/assets/library/2020-12-13/originals/beach001.jpg
- no_img: https://cdn.pixabay.com/photo/2018/01/03/09/09/book-3057904_960_720.png
