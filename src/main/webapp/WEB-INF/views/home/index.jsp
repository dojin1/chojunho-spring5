...
<!-- 상단 바로가기 클릭시 부드럽게 이동하는 외부 라이브러리 js 임포트(아래)-->
<script src="/resources/home/js/jquery.smooth-scroll.min.js"></script>
<!-- 화면을 초기화 시키는 reset 스타일 임포트:크로스브라이징  처리하기위해서-->
<!-- 크롬, IE, 엣지, 사파리, 파이어폭스 h1, p, u1, div 태그의 크기가 조금씩 다름 -->
<!-- 작업한 결과가 모든 브라우저(크로싱브라우징)에 똑같이 보이게 하기 위한 reset.css -->
...
/* PC용 메인페이지 스타일 지정(아래) 1066px~무한대까지 재정의 */
...
$(document).ready(function() {
	...
	$('.rollstop').click(function(){
		...
		if(slideAuto) {
			clearTimeout(slideAuto);//slideAuto 변수가 없다면, play_w 함수를 실행 중지.
		}
	});
	$('.rollplay a').click(function(){
		$(this).parent().hide();// a택의 부모 rollplay 영역 입니다.
		$('.rollstop').css('display','inline-block');
		play_w('right');//3초마다 슬라이드 이미지 액션 발생.
	});
